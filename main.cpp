#include <iostream>
using namespace std;
struct calendar{ // начало структуры
int day, month, year;
calendar (int a, int b, int c): // конструктор є1
day(a), month(b), year(c){}
int Date(){ // определени€ дн€ недели
int x, w, z, d;
x = ( 14 - month ) / 12;
w = year - x;
z = month + 12 * x - 2;
...
return d;
} // окончание определени€
}; // конец структуры
int d1, d2, m1, m2, y1, y2;
// начало расчЄта количесва лет между двум€ датами
...
// начало расчЄта количесва мес€цев между двум€ датами
...
// начало расчЄта количесва дней между двум€ датами
...
cin >> c;
calendar A(a, b, c);
if(A.Date() == 0)//вывод дн€ недели на экран
cout << "Sunday" << endl;
else if(A.Date() == 1)
cout << "Monday" << endl;
else if(A.Date() == 2)
