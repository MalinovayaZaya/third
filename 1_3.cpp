...
1)структура хранящая год,месяц, число и умеющая(с помощью соответствующего метода) определять по дате день недели

2)3 функции определяющие сколько лет, месяцев и дней прошло между двумя датами*/
#include <iostream>
using namespace std;
struct calendar{ 
int day, month, year;
...
w = year - q;
e = month + 12 * q - 2;
r = (7000 + ( day + w + w / 4 - w / 100 + w / 400 + (31 * e) / 12 )) % 7;
return r;
}
};
